...
This document describes how to deploy a Shiny app on Posit Connect, leveraging the [pins](https://cran.r-project.org/web/packages/pins/index.html) package to manage data. The application showcases single-cell RNA sequencing data analysis using the [scRNAseq](https://www.bioconductor.org/packages/release/data/experiment/html/scRNAseq.html) package to fetch data, perform quality control, normalization, and dimensionality reduction, displaying results with a Shiny interface.

Note that you may find more information and details in the Posit Connect documentation [Git-Backed Content section](https://docs.posit.co/connect/user/git-backed/).
...
- **Separate Repositories**: Maintain distinct repositories for your analytics and Shiny applications.
...
To keep all the code in a single repository for the simplicity of this demonstration, the analytics code described below is located in the [R/](https://github.com/ccb-hms/Git-Backed-Content/tree/master/R) directory of the [GitHub repository for this tutorial](https://github.com/ccb-hms/Git-Backed-Content). In general, it is best to follow best practices to keep this code in a separate repository or branch as specified above. 
...
The code below reads the data pinned on the Posit Connect server and generates the Shiny app. This code is available in the [Shiny/](https://github.com/ccb-hms/Git-Backed-Content/tree/master/Shiny) folder of the [GitHub repository for this tutorial](https://github.com/ccb-hms/Git-Backed-Content).
...
By following these steps, Posit Connect will automatically pull the contents of your repository, render the document, and display it on the server. By default, the Connect server checks your repository for new commits every 15 minutes, ensuring your deployed content remains up to date without any further manual interaction.
